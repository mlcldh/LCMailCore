...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc]initWithTitle:@"setting" style:(UIBarButtonItemStyleDone) target:self action:@selector(showSettingsViewController:)];
...
- (void)settingsViewControllerFinished:(SettingsViewController *)viewController {
    ...
    NSString *password = [[FXKeychain defaultKeychain] objectForKey:PasswordKey];
    NSString *hostname = [[NSUserDefaults standardUserDefaults] objectForKey:HostnameKey];
    NSLog(@"%@,%@,%@",self.imapSession.username,self.imapSession.password,self.imapSession.hostname);
    NSLog(@"%d,%d,%d",[username isEqualToString:self.imapSession.username],[password isEqualToString:self.imapSession.password],[hostname isEqualToString:self.imapSession.hostname]);
